  ...
  Visualizer = class Visualizer {
    constructor(size) {
	  ...
	  $('.start').on('click', (e) => {
		...
		var judge=$("#cold_hot").html();
		if (judge=="1"){
			this.stream.setAttribute("src", "static/music/happy.mp3");}
		else
		{this.stream.setAttribute("src", "static/music/sad.mp3");}
        return this.startAnalyser();

      });

document.getElementById("download").addEventListener("click", function(){
window.open("static/music/best.midi");
});
