...
window.setTimeout("window.location='index'",1200);
});

document.getElementById("btn_mix").addEventListener("click", function(){
    var form = $("<form>");   //定义一个form表单 

    form.attr('method','post');  
     form.attr('action','mix');  
    $('body').append(form);  //将表单放置在web中  
    form.submit();  //表单提交 
});

document.getElementById("btn_poetry").addEventListener("click", function(){
    var form = $("<form>");   //定义一个form表单 

    form.attr('method','post');  
     form.attr('action','poetry');  
    $('body').append(form);  //将表单放置在web中  
    form.submit();  //表单提交 
});

document.getElementById("btn_detect").addEventListener("click", function(){
    var form = $("<form>");   //定义一个form表单 

    form.attr('method','post');  
     form.attr('action','detect');  
    $('body').append(form);  //将表单放置在web中  
    form.submit();  //表单提交 
});
    });
