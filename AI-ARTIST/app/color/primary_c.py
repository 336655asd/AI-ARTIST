...
def getDist(preC, centroids):
    ...
    for i in range(k):
        h, s, v = preC[i]
        h1, s1, v1 = centroids[i]
        distance = math.sqrt(math.pow(s*math.cos(h) - s1*math.cos(h1), 2) + math.pow(s*math.sin(h) - s1*math.sin(h1), 2)+ math.pow(v - v1, 2))
        sum += distance
...
def show(im, count, centroids, k):
    ...
    #显示整个figure
    plt.show()
def temperature(pic_dir):
    data = loadImage(pic_dir)  # Can be many different formats.选择这种方式导入图片
    k = 10   # 设置k均值初始点个数
    ...
    for i in range(k):  # 因为有k个中心点
        ...
        centroids[i] = r, g, b
        t=[str(int(r)),str(int(g)),str(int(b))]
        line=','.join(t)
        lines.append(line)
        print i, r, g, b
    with open('app/color/temputre.txt','w') as f:
        f.write('\n'.join(lines))
